...
// Grab the p element which will display the text
const factsText = document.getElementById("facts-text");

// Set the initial index to the first element of the array
let factIndex = 0;

// Function is invoked if the user clicks the right arrow
const nextFact = () => {
  // If it is the last element of the array
  // set the index to the first element
  if (factIndex === factsList.length - 1) {
    factIndex = 0;
  } else {
  ...
  // Update the DOM with a new fact
  factsText.innerText = factsList[factIndex];

  // Add the slide right class to animate the transition
  factsText.classList.add("slide-right");

  // Set a timer to remove that class after the animation is complete
  setTimeout(() => factsText.classList.remove("slide-right"), 900);
};

// The inverse of the previous function (left arrow)
const lastFact = () => {
  if (factIndex === 0) {
    factIndex = factsList.length - 1;
  } else {
    factIndex -= 1;
  }
  factsText.innerText = factsList[factIndex];
...
// Add an event listener so the user can
// use the left and right arrows as well
// as pressing the arrow icons
